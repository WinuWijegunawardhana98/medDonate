...
class PatientPrescriptionList : AppCompatActivity() {

    private lateinit var auth: FirebaseAuth
    private lateinit var db: FirebaseFirestore
    private lateinit var prescriptionAdapter: PrescriptionCrudAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val listView = findViewById<ListView>(R.id.prescriptionCrudListView)
        prescriptionAdapter = PrescriptionCrudAdapter(this)
        listView.adapter = prescriptionAdapter

        // Fetch prescriptions with "not accepted" status
        db.collection("prescriptions")
            .whereEqualTo("userRef", currentUserRef)
